...
namespace imperazim\vendor\simplepackethandler\interceptor;
...
use imperazim\vendor\simplepackethandler\utils\Utils;
...
final class PacketInterceptorListener implements IPacketInterceptor {
  ...
  */
  ...
  public function interceptOutgoing(Closure $handler) : IPacketInterceptor {
    $this->outgoing_handlers[self::getPidFromHandler($handler,
      ClientboundPacket::class)][spl_object_id($handler)] = $handler;
    $this->outgoing_event_handler ??= Server::getInstance()->getPluginManager()->registerEvent(DataPacketSendEvent::class,
