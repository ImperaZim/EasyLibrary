...
use Closure;
use LogicException;
use imperazim\vendor\invmenu\inventory\SharedInvMenuSynchronizer;
use imperazim\vendor\invmenu\session\InvMenuInfo;
use imperazim\vendor\invmenu\session\network\PlayerNetwork;
use imperazim\vendor\invmenu\transaction\DeterministicInvMenuTransaction;
...
use imperazim\vendor\invmenu\transaction\SimpleInvMenuTransaction;
...
class InvMenu implements InvMenuTypeIds{
	...
	public static function create(string $identifier, ...$args) : InvMenu{
		return new InvMenu(InvMenuManager::getTypeRegistry()->get($identifier), ...$args);
	}
	...
	final public function sendTo(Player $player, ?string $name = null, ?Closure $callback = null) : void{
		...
		$session = InvMenuManager::getPlayerManager()->get($player);
	...
	public function onClose(Player $player) : void{
		...
		InvMenuManager::getPlayerManager()->get($player)->removeCurrentMenu();
	}
}
