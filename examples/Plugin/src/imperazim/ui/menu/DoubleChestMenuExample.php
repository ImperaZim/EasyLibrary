...
use imperazim\vendor\invmenu\types\InvMenuDoubleChest;
...
/**
* Class DoubleChestMenuExample
* @package imperazim\ui\menu
*/
final class DoubleChestMenuExample extends Menu {

  /**
  * Generates and sends the menu to the player.
  */
  public function structure(): IMenu {
    return new InvMenuDoubleChest(
  ...
  */
  private function getMenuName(): string {
    ...
    return $file->get('double_chest_menu.title', '');
  }
  ...
  */
  private function getContents(): array {
    $contents = [];
    $file = UiManager::getFile('forms');
    $contentList = $file->get('double_chest_menu.contents', []);
  ...
  */
  private function handleClose(Player $player, Inventory $inventory): void {
    $player->sendMessage('Double Chest Menu closed!');
  }
}